create or replace package body oracle_plsql_test
as

  type my_internal_type is record (
    f_name varchar2(255),
    sal number
  );

  type my_internal_type_tab is table of my_type;

  gc_internal_a constant number := 1;
  gc_internal_b constant number := 2;
   ...
   * @example
   * begin
   *   if oracle_plsql_test.is_developer then
   ...
   * @example
   * begin
   *   if oracle_plsql_test.is_developer_yn = 'Y' then
   *     dbms_output.put_line('Developer mode');
   *   else
   *     dbms_output.put_line('Non-Dev mode');
   *   end if;
   ...
   *
   * begin
   *   if oracle_plsql_test.is_session_valid(p_session_id => :app_session) then
   ...
   *
   * begin
   *   if oracle_plsql_test.is_session_valid_yn(p_session_id => :app_session) = 'Y' then
   *     dbms_output.put_line('Session is active');
   *   else
   *     dbms_output.put_line('Session is inactive');
   *   end if;
   ...
   *
   * begin
   *   oracle_plsql_test.join_session(
   ...
   *
   * begin
   *   oracle_plsql_test.trim_page_items(p_page_id => :app_page_id);
   * end;
   *
   * @issue 24
   *
   ...
   * @example
   * begin
   *   if oracle_plsql_test.is_page_item_rendered(p_item_name => 'P1_EMPNO') then
